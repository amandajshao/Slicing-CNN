...
#ifdef USE_MPI
  const int total_batch_size = this->layer_param_.window_data_param().batch_size();
  if (total_batch_size % Caffe::mpi_size() != 0) {
    LOG(FATAL) << "Batch size (" << total_batch_size << ") should be divisible by "
                  "the number of MPI processes (" << Caffe::mpi_size() << ")";
  }
  this->layer_param_.mutable_window_data_param()->set_batch_size(
      total_batch_size / Caffe::mpi_size());
#endif
